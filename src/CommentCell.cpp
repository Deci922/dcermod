...
void download_list() {
	...
	if (!downloaded) {
        ...
        m_listener.setFilter(req.get("https://raw.githubusercontent.com/Deci922/dcermod/main/names.txt"));
		}
	}
					...
					if (m_comment->m_modBadge < 1) {
						...
						if (player_comment_big_area) {
							...
							for (int i = 0; i < text_holder->getChildrenCount(); i++) {
								...
								if (player_comment_big->getColor() == ccColor3B{255,255,255}) player_comment_big->setColor({ 153, 255, 255 });
							}

						}
						else {
							if (player_comment_small->getColor() == ccColor3B{255,255,255}) player_comment_small->setColor({ 153, 255, 255 });
						}
