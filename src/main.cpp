...
class $modify(YouTuberAlert, ProfilePage) {
	...
	void setupPageInfo(gd::string name, char const* chars) {
		...
		for (const auto& names : YouTubers) {
			...
			if (lower_names == lower_player_name) {
				...
				m_fields->icon->setID("deci922.dcermod/YouTuber-Badge");

				if (!static_cast<CCMenu*>(usernameMenu->getChildByID("deci922.dcermod/YouTuber-Badge"))) {
	...
	void found_youtube(CCObject*) {
		FLAlertLayer::create("Daily Chatter","This user is a <cr>prominent member</c> of the <cy>Geometry Dash</c> <cg>Daily Chat!</c>!","OK")->show();
